...
export class Manager {
  ...
  async updateStatus(key, status) {
    const statusId = this.constructor.enum(status);
    return await this.update(key, { status, statusId, updatedAt: Date.now() });
  }
...
export class Error extends Manager {
  constructor() {
...
export class Granule extends Manager {
  ...
  static async buildRecord(collectionName, pdrName, granuleId, files, collectionObj = null) {
    ...
    granuleRecord.statusId = this.enum(granuleRecord.status);
  ...
  async ingestCompleted(key, files) {
    ...
    updatedRecord.statusId = this.constructor.enum(updatedRecord.status);
