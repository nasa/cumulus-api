...
export class Manager {
  static recordIsValid(item, schema = null) {
    if (schema) {
      const ajv = new Ajv({ useDefaults: true, v5: true, removeAdditional: 'all' });
...
export class Collection extends Manager {
  static recordIsValid(item, schema = null) {
    ...
    recipeFiles.forEach((rf) => {
      ...
      if (!test) throw new Error('inputFiles don\'t match files keys');
    });
    ...
    if (!test) throw new Error('recipe order items don\'t match recipe keys');
    ...
    const validation = new RegExp(item.granuleDefinition.granuleId);
    test = validation.test(match[1]);

    if (!test) {
      throw new Error('granuleId regex cannot validate output of granuleIdExtraction');
    }
    ...
    if (!test) {
      throw new Error('neededForProcessing items don\'t match files keys');
    }
  }
