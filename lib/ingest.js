...
import { syncUrl, downloadS3Files, promiseS3Upload } from 'gitc-common/aws';
...
class PdrIngest {
    ...
    try {
      ...
      for (const chunk of chunks) {
        ...
        if (!this.collection) {
          ...
          await this._getCollection(chunk[0].files[0].filename);
        }


        const granules = chunk.map(g => ({
          granuleId: null,
  ...
  async _queueForDownload(granule) {
    ...
    if (!isAlreadyIngested) {
      ...
      const p = new Pdr();
      return p.updateStatus({ pdrName: granule.pdrName }, 'parsed');
    }
  }
...
const ftpMixin = superclass => class extends superclass {
  ...
  async _download(host, path, filename) {
    ...
    return new Promise((resolve, reject) => {
      this.ftpClient.on('error', (err) => {
        const e = {
          message: `FTP download failed for ${this.host}: ${err.message}`,
          details: err
        };
      ...
      this.ftpClient.on('ready', () => {
        this.ftpClient.get(join(this.path, filename), (err, stream) => {
      ...
      this.ftpClient.connect(this.ftpOptions);
    });
  }
  ...
  _list() {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.ftpClient.on('ready', () => {
        this.ftpClient.list(this.path, (err, list) => {
