...
class GranuleIngest {
  ...
  constructor(granule, username, password) {
    ...
    logDetails.granuleId = this.granuleId;
    logDetails.pdrName = this.granule.pdrName;
    logDetails.provider = this.granule.provider;
  }
  ...
  async ingest(dispatch = true) {
    try {
      ...
      const errorMessage = `Ingesting ${this.granuleId} failed: ${e.message}`;
      log.error(errorMessage, logDetails);
      log.error(e, logDetails);
      const g = new Granule();
      await g.hasFailed({ granuleId: this.granuleId }, errorMessage, 'ingest');
    }
  }
...
/**
 * This is a base class for writing PDR's PAN/PDRD messages
 *
 * @class
 * @abstract
 */
...
class Pan {
  constructor(provider, pdrName, username, password) {
    if (this.constructor === Pan) {
      throw new TypeError('Can not construct abstract class.');
    }

    this.host = provider.host;
    this.path = provider.path;
    this.panFolder = provider.panFolder;
  ...
  async write(message, type) {
    const panName = this.pdrName.replace('PDR', type);
    await this._write(this.host, this.panFolder, panName, message);
  }
}
...
class PdrIngest {
  ...
  async _setProviderError(err) {
    ...
    await p.hasFailed({ name: this.provider.name }, err);
...
const ftpMixin = superclass => class extends superclass {
  ...
  async _write(host, path, filename, body) {
    // let's stream to file
    const uri = urljoin(host, path, filename);
    log.info(`Writing to ${uri}`, logDetails);
...
/**
 * Write Pan and PDRD to a FTP endpoint.
 *
 * @class
 */

export class FtpPan extends ftpMixin(Pan) {}
