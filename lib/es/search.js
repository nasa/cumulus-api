...
export class Search {
  ...
  async get(id, histogram = false) {
    try {
      ...
      if (histogram) {
        result._source = await this.getStats([result._source])[0];
      }
  ...
  async histogram(query, type, aggrField, interval = 1) {
    const params = {
