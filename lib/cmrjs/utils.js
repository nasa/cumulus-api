...
export function getUrl(type) {
  ...
  const host = getHost();
  const env = process.env.CMR_ENVIRONMENT;
  const provider = secrets.CMR_PROVIDER;
...
export async function updateToken() {
  ...
  const tokenData = {
    token: {
      username: secrets.CMR_USERNAME,
      password: secrets.CMR_PASSWORD,
      client_id: secrets.CMR_CLIENT_ID,
      user_ip_address: ip,
      provider: secrets.CMR_PROVIDER
    }
  };
...
export async function tokenIsValid(token) {
  ...
  const resp = await got.put(
    `https://cmr.uat.earthdata.nasa.gov/ingest/providers/${process.env.CMR_PROVIDER}/collections/CMRJS_TOKEN_TEST`,
    {
      body: null,
      headers: {
        'Echo-Token': token,
